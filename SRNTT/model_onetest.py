    ...
    def train(
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in xrange(num_epochs):
                ...
                files.save_npz(
                    save_list=self.net_srntt.all_params,
                    name=join(self.save_dir, MODEL_FOLDER, str(epoch+step+1)+'_' +SRNTT_MODEL_NAMES['conditional_texture_transfer']),
                    sess=sess)
                files.save_npz(
                    save_list=self.net_d.all_params,
                    name=join(self.save_dir, MODEL_FOLDER, str(epoch+step+1)+'_'+SRNTT_MODEL_NAMES['discriminator']),
                    sess=sess)
